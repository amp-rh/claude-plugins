---
name: mpitt_single_rule_reviewer
description: |
  Enforces exactly one mpitt ruleId per task on listed paths; JSON violations only; no fixes, no git, no shell.

  ```example
  Orchestrator dispatches Task with one ruleId and verbatim rule text to audit only the PR-touched *-commands.sh paths listed in files[].
  ```

  ```example
  Per-rule wave audit on ci-operator/config/**/*.yaml entries: supply artifact_type yaml and return JSON violations for that single rule across allowed paths.
  ```
