---
name: sippy_interop_jobs
description: |
  Fetches OpenShift CI periodic job run data, pass rates, and failure history from the Sippy REST API for interop testing triage.

  Example:
  ```
  User: What are Sippy pass rates for interop jobs on OCP 4.21?
  ```

  Example:
  ```
  User: Show recent rosa-hypershift run results and Prow links for 4.20.
  ```

  Example:
  ```
  User: Find OPP AWS periodic jobs with low pass rate or regressions in Sippy.
  ```
tools:
  - Bash
  - Read
